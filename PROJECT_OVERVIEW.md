...
| [editor/src/browser-mock-host.ts](editor/src/browser-mock-host.ts) | Alpha/Beta fixture、内存保存、导航和部分引用命令；不是完整存储实现 |
...
当前建表定义包含以下 13 张表；`GetTableCounts()` 的诊断结果未包含 `save_transactions`，不能以诊断输出的键数推断实际表数。
...
`blocks.scope_type='canonical'` 表示真实内容；`reference_instance` 表示实例专属新增内容，`scope_id` 关联实例。`parent_id` 与 `position` 构成树与排序；当前前端常以补零字符串顺序值排列，不是 CRDT 排序方案。
