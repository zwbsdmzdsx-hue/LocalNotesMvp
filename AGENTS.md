...
- 当前阶段只完成新版网页端（`editor/`）的功能、交互和视觉验收；网页端的开发服务器、浏览器 Mock 与 `web/tests/editor/` 是本阶段的主要验证入口。
- 暂不继续扩展桌面兼容编辑器、Avalonia 窗口、C# 宿主、SQLite 落盘和 `web/src/` / `web/dist/`。这些已有代码和未提交改动必须保留，但不能因为协议或桌面草稿已经存在就扩大本轮范围。
- 只有在新版网页端功能和视觉完成并通过对应验证后，才开始把已确认的能力同步到桌面端；桌面端同步属于后续阶段。
...
- 增加或修改功能前，先找出它的共同数据来源，以及所有使用它的入口和视图（如正文、Canvas、右栏、预览、源码、历史与导出）。按本轮交付范围同步处理受影响的路径，并验证关键交互，不能只让最先发现问题的界面看起来正常。
